...
const { v$, formErrors, validateState, touchOnChange, isSubmitInProgress, startSubmit, stopSubmit } = useEsForms(
    rules,
    state,
);
    ...
    <b-container>
        <b-row>
            <b-col>
                ...
                <h2>UX Guidelines</h2>
                <ul>
                    <li>Validation should initially trigger <em>on blur</em>.</li>
                    <li>Subsequent validation should trigger <em>on change</em>.</li>
                    <li>Validation for password fields should always trigger <em>on change</em>.</li>
                    <li>
                        You'll also want to provide feedback from a server response. For guidance on how to do this,
                        see the
            ...
            <b-col>
                ...
                <es-collapse class="mt-500">
                    ...
                    <p>
                        ...
                        <code>mixins</code> of your component. Mixins are no longer recommended by Vue, and are not
                        compatible with the composition API. Therefore, these form utilities are now provided as a
                        <nuxt-link to="https://vuejs.org/guide/reusability/composables"> composable </nuxt-link>.
                    </p>
                    <p>
                        To use the composable, call <code>useEsForms</code> within your script, as shown in the
                        documentation source. Parameters are your validation rules and the reactive state that includes
                        your form. You will get back all the functions and reactive refs that were previously
                        accessible on <code>this</code> as a result of using the mixin. You will also get back the
                        Vuelidate object, now called <code>v$</code> instead of <code>$v</code> due to a restriction
                        within Vue 3.
                    </p>
                    <p>
                        If you're still using the options API, please refer to the
                        <nuxt-link
                            to="https://vuejs.org/guide/reusability/composables#using-composables-in-options-api">
                            Vue documentation
                        </nuxt-link>
                        for using a composable with the options API.
                    </p>
                    <p>
                        Note that while the API for functions in this composable have not changed from the version in
                        the old mixin, there <b>are</b> upstream changes in Vuelidate which may require manual
                        handling. At a minimum, <code>$v</code> is now <code>v$</code> as previously mentioned. Please
                        refer to the
