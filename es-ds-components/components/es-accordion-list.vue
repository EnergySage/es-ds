...
<template>
    <Accordion
        :multiple="allowMultipleExpand" :active-index="activeIndex" :pt="{
            root: {
    ...
    >
        ...
        <template #expandicon />
        <AccordionTab
            v-for="(tab, index) in accordionTabs" :key="index" :pt="{
                root: ({ context }) => ({
                    class: [
        ...
        >
            <template #header>
                <span role="heading" aria-level="3" :class="{ 'h3 mb-0': variant === 'minimal' }">
                    <component :is="item" v-for="item in tab.title()" />
                </span>
                <icon-chevron-down class="es-accordion-icon flex-shrink-0 ml-200" />
            </template>
            <component :is="item" v-for="item in tab.default()" />
        </AccordionTab>
    </Accordion>
</template>
