...
import { useEsAutocompleteVisibleRows } from './autocomplete-visible-rows';
...
describe('useEsAutocompleteVisibleRows', () => {
    it('renders the number of whole rows that fit the height limit', async () => {
        ...
        const fit = withSetup(() => useEsAutocompleteVisibleRows(contentEl, suggestions, 10));
        await fit.remeasure();
    ...
    it('ignores viewport events while inactive: a closed, still-mounted panel never re-measures on scroll', async () => {
        ...
        const fit = withSetup(() =>
            useEsAutocompleteVisibleRows(contentEl, suggestions, 10, { active: () => active.value }),
        );
        await fit.remeasure();
        expect(fit.visibleSuggestions.value).toHaveLength(2);
    ...
    it('beforeMeasure runs in the same pass, so a limit it writes drives that count', async () => {
        ...
        const fit = withSetup(() =>
            useEsAutocompleteVisibleRows(contentEl, suggestions, 10, {
                beforeMeasure: () => {
                    container.style.maxHeight = '70px';
                },
            }),
