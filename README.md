...
3. `make build-scss-pkg` (will build _es-bs-base_, which must happen before building _es-vue-base_ because there's lines like `@import '~@energysage/es-bs-base/scss/includes';` in _es-vue-base_ that will not compile without having the _es-bs-base_ package built first)
